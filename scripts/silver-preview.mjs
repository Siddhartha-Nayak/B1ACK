...
await seed(page, 0);
await page.screenshot({ path: 'artifacts/silver-empty.png' });
...
await page.screenshot({ path: 'artifacts/silver-workspace.png' });
await cdp.send('Emulation.setDeviceMetricsOverride', {
  width: 390,
  height: 844,
  deviceScaleFactor: 1,
...
await page.screenshot({ path: 'artifacts/silver-mobile.png' });
await cdp.send('Emulation.clearDeviceMetricsOverride');
await fs.writeFile(
  'artifacts/silver-theme-results.json',
  JSON.stringify({
    liveShells: 4,
    narrowWidth: 390,
    overflow: false,
